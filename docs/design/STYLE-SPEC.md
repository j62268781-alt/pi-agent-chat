...
| 字形       | 控制条**自己那四个控件**内的 codicon 取 `--pi-icon-md` = 14px，保持宿主 22px 行上 12px 的字形/控件比；两个弹窗（模型、权限）在 DOM 里就挂在控制条内部，所以这条规则必须点名控件、不能用后代选择器，否则弹窗里的图标会被一起压到 14px |
...
**字形盒要跟字号同尺寸**：`.icon-btn .codicon` 是工具栏的规格（盒 16px），控制条把字号压到
14px 时必须**把盒宽高一起改**——否则 14px 的行盒贴在 16px 盒的顶边，`+` 与发送箭头实测比控件
中心高 1.5–2px（而权限药丸的锁没被改过盒，正好居中）。
