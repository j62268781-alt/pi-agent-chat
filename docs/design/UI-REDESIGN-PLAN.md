...
| 区域       | 问题                                       | 影响                                                                                     |
| ---------- | ------------------------------------------ | ---------------------------------------------------------------------------------------- |
| **对话区** | 早期方案使用 IM 气泡（左右分栏、圆角气泡） | **范式错误** —— Agent 对话不是人与人聊天，气泡割裂了代码块 / 工具调用 / 推理的连续阅读流 |
...
1. **转写稿（transcript）而非聊天**：左对齐、单列阅读流，像「终端 + 文档」的混合体，而非 IM。
...
**废弃**：左右分栏气泡、头像+气泡尾巴、按发送方对齐。
**采用**：单列、左对齐、全宽阅读列；**每一轮 = 一条带 gutter 的记录**。
