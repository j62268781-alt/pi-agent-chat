...
import { THEMES, type PreviewTheme } from "./themes.generated";

const params = new URLSearchParams(location.search);
const theme = THEMES.find((entry) => entry.id === params.get("theme")) ?? THEMES[0];
if (!theme) throw new Error("preview/themes.generated.ts is empty — run gen-theme-vars.mjs");

/** The host injects these alongside the theme colours; the preview supplies
 *  stand-ins so the typography is not measuring a browser default instead. */
...
function applyTheme(next: PreviewTheme): void {
