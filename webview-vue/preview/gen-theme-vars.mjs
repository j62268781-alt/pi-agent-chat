// Materialize the real colour values of VS Code's built-in themes as
// `--vscode-*` custom properties, so the design preview runs on the same
// numbers the webview receives from the host instead of hand-picked swatches.
//
//   node preview/gen-theme-vars.mjs
//
// Two sources, both read from the local VS Code install:
//
