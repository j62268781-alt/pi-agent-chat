...
import { findPiBinary, normalizePiSpawnTarget } from "../pi/process.ts";
...
export async function detectPiVersion(piPath: string): Promise<string> {
  try {
    ...
    if (stderr && stderr.trim()) {
      console.error(`[pi-agent-chat] detectPiVersion stderr: ${stderr.trim()}`);
    }
    ...
    console.error(`[pi-agent-chat] detectPiVersion failed (piPath=${piPath}):`, err);
  }
  return "(unknown)";
}
