...
export function readAuthJson(): AuthJson {
  ...
  console.log("[pi-agent-chat] readAuthJson: path =", path, "exists =", existsSync(path));
  if (!existsSync(path)) return { version: 1 };
  try {
    const content = readFileSync(path, "utf8");
    const data = JSON.parse(content) as AuthJson;
    console.log(
      "[pi-agent-chat] readAuthJson: loaded keys =",
...
export async function refreshModelRegistry(): Promise<void> {
  try {
    ...
    console.error("[pi-agent-chat] refreshModelRegistry failed:", err);
  }
}
...
> {
  try {
    ...
    console.error("[pi-agent-chat] getApiKeyProviderStatuses failed:", err);
    return [];
  }
}
