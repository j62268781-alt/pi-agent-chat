...
/** Invalidate the cached pi binary resolution; call when `pi-agent-chat.path` changes. */
...
export function createRpcEnvironment(
  ...
  const userEnv =
    vscode.workspace.getConfiguration("pi-agent-chat").get<Record<string, string>>("env") ?? {};
