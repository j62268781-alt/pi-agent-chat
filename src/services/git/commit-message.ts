...
import { getGitDiff } from "./git-utils.ts";
...
async function performCommitMsgGeneration(gitDiff: string, inputBox: any) {
  ...
  try {
    vscode.commands.executeCommand("setContext", "pi-agent-chat.isGeneratingCommit", true);
    const config = vscode.workspace.getConfiguration();

    // Get custom prompts or use defaults
    const customSystemPrompt = config.get<string>("pi-agent-chat.commitMessagePrompt", "");
    const PROMPT = {
    ...
    const commitLanguage = config.get<string>("pi-agent-chat.commitLanguage", "English");
    ...
    const commitModelRaw = config.get<string>("pi-agent-chat.commitModel", "");
    ...
    if (commitModelRaw.trim()) {
      ...
      if (!parsed) {
        throw new Error(
          t(
            'Invalid "pi-agent-chat.commitModel": "{0}". Expected format "provider/model".',
            commitModelRaw,
          ),
        );
      }
    ...
    vscode.commands.executeCommand("setContext", "pi-agent-chat.isGeneratingCommit", false);
    if (abortListener) {
