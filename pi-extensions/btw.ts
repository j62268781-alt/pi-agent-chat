    ...
    handler: async (args, ctx) => {
      ...
      if (ctx.mode === "tui") {
        ...
        if (typeof answer === "object") {
          ctx.ui.notify(`/btw failed: ${answer.error}`, "error");
          return;
        }
      ...
      await askAndShowInWebview(question, ctx);
    },
  });
}

type AskFailure = { error: string };
...
async function askSideQuestion(
  question: string,
  ctx: ExtensionCommandContext,
): Promise<string | AskFailure | undefined> {
  return ctx.ui.custom<string | AskFailure | undefined>((tui, theme, _keybindings, done) => {
    const loader = new BorderedLoader(tui, theme, `Answering /btw with ${ctx.model!.id}...`);
    loader.onAbort = () => done(undefined);

    fetchAnswer(question, ctx, loader.signal)
      .then(done)
      .catch((error: unknown) => {
...
function sanitizeSingleLine(text: string) {
  return text
    .replace(/[\r\n\t]/g, " ")
    .replace(/[\u0000-\u001f\u007f-\u009f]/g, "") // eslint-disable-line no-control-regex -- intentional: strip ASCII control chars (C0/C1), keep \r\n\t handled above
    .replace(/ +/g, " ")
    .trim();
}
